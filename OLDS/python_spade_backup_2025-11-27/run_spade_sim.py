...
class CommandSenderAgent(spade.agent.Agent):
    ...
    class CommandSender(spade.behaviour.OneShotBehaviour):
        async def run(self):
            ...
            print(f"\n[COMANDO] Enviando: {self.agent.command} para {get_sender_name(self.agent.target_jid)}...")
            await self.send(m)
...
async def send_command(supervisor_jid: str, command: str, wait_time: float = 0.5):
    """Cria e inicia um agente temporário para enviar o comando (correção de wait_until_finished)."""
    # Usamos uma JID temporária que não entra em conflito
    sender_jid = generate_jid("controller_temp", random.randint(1000, 9999)) 
    sender_agent = CommandSenderAgent(sender_jid, PASSWORD, supervisor_jid, command)
    
    await sender_agent.start(auto_register=True)
    
    # CORREÇÃO: Usamos asyncio.sleep() em vez de wait_until_finished, pois o OneShotBehaviour
    # já garante o envio antes de terminar.
...
async def main():
    ...
    voters = []
    voter_party_map = {} # Mapeamento JID para Party (essencial para o Supervisor)
    ...
    # 2. Media
    # CORREÇÃO: Incluir authority_jid na instanciação
    media = MediaAgent(
        media_jid,
        PASSWORD,
        supervisor_jid=sup_jid,
        authority_jid=auth_jid, # NOVO ARGUMENTO POSICIONAL ADICIONADO
        voter_jids=[str(v.jid) for v in voters], # Inicializa a Media com a lista de eleitores
    )
    ...
    print(f"\n[EXEC] Simulação rodará automaticamente até T{cfg.TOTAL_TICKS} por {total_run_time:.1f} segundos.")
